...
- Оставляйте `logQueryValues=false`. Тогда в query log редактируются строковые,
  ...
  всегда.
- Query log записывается асинхронно через ограниченную общую очередь. При
  переполнении новая диагностическая строка отбрасывается и добавляется ошибка
  `AgtySQL`, но JDBC-выполнение не блокируется; operational tooling должно
  учитывать небольшую задержку появления строки в файле.
